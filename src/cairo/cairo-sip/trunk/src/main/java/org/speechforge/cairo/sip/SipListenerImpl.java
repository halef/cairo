...
public class SipListenerImpl implements SipListener {
    ...
    /*
        ...
        // String event = "RECEIVED_SIP_" + request.getMethod() + "_REQUEST";
        // String appName = System.getenv("APP_NAME");
        // String endpoint = appName.equals("CAIRO") ? "cairo-event" : "jvxml-event";
        // DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-d H:m:s:S");
        // LocalDate date = LocalDate.now();
        // String happend_at = date.format(formatter);
        // String hevent = "";
        // if (appName.equals("CAIRO")) {
        ...
        //              .add("server_ip", System.getenv("IP"))
        //              .add("client_session_id", HalefDbWriter.getClientSessionId())
        //              .add("jvxml_session_id", HalefDbWriter.getJVXMLSessionId())
        //              .add("jvxml_call_id", HalefDbWriter.getJVXMLCallId())
        //              .build().toString();
        // }
        // HalefDbWriter.logGanesha(endpoint, hevent);
    ...
    public void processResponse(ResponseEvent responseEvent) {
        ...
        Iterator headers1 = response.getHeaderNames();
        while (headers1.hasNext()) {
           sb1.append("\n");
           sb1.append(response.getHeader((String) headers1.next()).toString());
        }
       
        byte[] contentBytes1 = response.getRawContent();

        if (contentBytes1 == null) {
           sb1.append("\nNo content in the response.");
        } else {
           String contentString = new String(contentBytes1);
           sb1.append("\n");
           sb1.append(contentString);
        } 
    ...
    public void processInvite(RequestEvent requestEvent) {
		...
		if (dialog != null) {
		       ...
		       MDC.put("dialogId",did);
			       ...
			       System.out.println("SIP Call ID (dialog id) "+ dialog.getDialogId());
