...
public class RTPRecogChannel {
	...
	public synchronized void recognize(RecogListener listener, // !AI! this is a call to a recognition device
		...
		try {
			...
			_logger.debug("PATRICK HACK: Return engine back if exists then borrow one");
	...
	private class Listener extends RecogListenerDecorator {
		...
		public void recognitionComplete(RecognitionResult result) {
			_logger.debug("recognitionComplete() (RTPRecogChannel.java)");
		...
		public void recogFakeResult(RecognitionResult result) {
			_logger.debug("Timeout code rtprecogchannel");
