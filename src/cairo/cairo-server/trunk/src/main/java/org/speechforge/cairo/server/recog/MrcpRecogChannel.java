...
public class MrcpRecogChannel extends MrcpGenericChannel implements RecogOnlyRequestHandler {
    ...
    public synchronized MrcpResponse recognize(UnimplementedRequest request, MrcpSession session){
        ...
        if (_state == RECOGNIZING) {
            ...
            if (request.hasContent()) {
                ...
                if (contentType.equalsIgnoreCase("application/wfst") || contentType.equalsIgnoreCase("application/x-jsgf") || 
		    	contentType.equalsIgnoreCase("application/arpa") || contentType.equalsIgnoreCase("application/srgs+xml")) {
                    _logger.debug("processing " + contentType);
		    if (contentType.equalsIgnoreCase("application/srgs+xml")) {
			_logger.info("Content type 'application/srgs+xml' not supported");
			_logger.info("Using fallback to 'application/x-jsgf'");
			contentType = "application/x-jsgf";
             	    }	
    ...
    private class Listener implements RecogListener {
        ...
        public void recognitionComplete(RecognitionResult result) {
        	...
        	_logger.debug("...and past the synchronized block (MrcpRecogChannel.java)");
            try {
...
//		    event.removeContent();
//                    event.setContent("text/plain", null, "null");  //This is a hack by david@suendermann.com
                } else {
                    CompletionCause completionCause = new CompletionCause((short) 0, "success");
                    event.addHeader(MrcpHeaderName.COMPLETION_CAUSE.constructHeader(completionCause));
        ...
        public void recogFakeResult(RecognitionResult result) {
                _logger.debug("speech complete event...");
            synchronized (MrcpRecogChannel.this) {
                _state = RECOGNIZED;
            }
