...
//AI:added  import edu.cmu.sphinx.jsapi.JSGFGrammar;
...
public class StandaloneRecogClient extends RecogListenerDecorator {
    ...
    public synchronized void recognitionComplete(RecognitionResult result) {
        _logger.debug("recognitionComplete() (StandaloneRecogClient.java)");
        _result = result;
        if (_beep) {
            _toolkit.beep();
        }
...
//AI:added	    jsgfGrammarManager = (JSGFGrammar) cm.lookup("grammar");
//AI:          SphinxRecEngine engine = new SphinxRecEngine(cm,1);

            _logger.info("Loading grammar file...");
//AI:alternated          jsgfGrammarManager.loadJSGF(grammarLocation);

//AI:          StandaloneRecogClient client = new StandaloneRecogClient(engine);
//AI:          RecognitionResult result = client.doRecognize(MICROPHONE, examplePhrase);
